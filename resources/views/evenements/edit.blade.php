...
<div class="container mt-5">
    <form method="post" action="{{ route('evenements_edit', $evenement) }}">
      @csrf
        ...
        <div class="form-group">
          ...
          <select class="form-control @error('organisme_id') is-invalid @enderror" name="organisme_id">
            @foreach($organismes as $organisme)
              <option value="{{$profil->id}}" @if($organisme->id == $evenement->organisme_id) selected="selected" @endif>{{$organisme->nom}}</option>
            @endforeach
          </select>
          @error('organisme_id')
          <div class="invalid-feedback">{{ $message }}</div>
