...
<table class="table table-hover pt-4">
    ...
    @php(array_push($done, $evenement['start']))
      <tr class="row m-0">
        ...
        <td class="col-5">@if (!$evenement['active']) <i class="fas fa-circle" style="color: red"></i> @endif<a href="javascript:void(0)" class="popupEvent" data-url="{{ route('evenement_popup', ['id' => $evenement['id']]) }}"><strong>{{$evenement['simpleTitle']}}</strong><a></td>
        <td class="col-5">{!! $evenement['organismes'] !!}</td>
